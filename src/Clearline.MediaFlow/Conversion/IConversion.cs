...
public interface IConversion
{
    ...
    ///     Should be output file overwritten or not. If not overwrite and file exists conversion will
    ...
    /// <exception cref="TaskCanceledException">Occurs when task was canceled.</exception>
    Task<IConversionResult> Start(CancellationToken cancellationToken = default);

    /// <summary>
    ///     Start an FFmpeg process with specified arguments
    /// </summary>
    /// <param name="arguments">FFmpeg parameters eg. "-i sample.mp4 -v 0 -vcodec mpeg4 -f mpegts udp://127.0.0.1:23000"</param>
    /// <returns>Conversion result</returns>
    /// <exception cref="ConversionException">Occurs when FFmpeg process return error.</exception>
    /// <exception cref="ArgumentException">Occurs when no FFmpeg executables were found.</exception>
    /// <exception cref="InvalidOperationException"></exception>
    /// <exception cref="ObjectDisposedException"></exception>
    Task<IConversionResult> Start(string arguments);
    ...
    /// <exception cref="ObjectDisposedException"></exception>
    /// <exception cref="TaskCanceledException">Occurs when task was canceled.</exception>
    Task<IConversionResult> Start(string arguments, CancellationToken cancellationToken);

    /// <summary>
    ///     Add additional parameters for the conversion (They must be well-formed)
    /// </summary>
    /// <param name="parameter"> Parameter to set</param>
    /// <param name="argumentPosition">Position of parameter</param>
    /// <returns>IConversion object</returns>
    IConversion AddParameter(string parameter, ArgumentPosition argumentPosition = ArgumentPosition.PostInput);
    ...
    /// <summary>
    ///     Use hardware acceleration. This option set -threads to 1 for compatibility reasons. This should be used with proper
    ///     codec (e.g. -c:v h264_nvenc or h264_cuvid)
    /// </summary>
